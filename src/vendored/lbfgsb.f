...
c  MT DEVIATION (see README.vendored): the upstream write(6,*) here is not
c  guarded by iprint and lnsrlb has no iprint argument to guard it with, so it
c  printed to stdout from inside a shared library on every non-descent step.
c  info = -4 already reports the condition to the caller through the normal
c  reverse-communication path.
c           write(6,*)' ascent direction in projection gd = ', gd
            info = -4
            return
         endif
      endif
...
c  MT DEVIATION (see README.vendored): these two upstream write(6,*) calls are
c  not guarded by iprint (subsm HAS an iprint argument, so this is an upstream
c  oversight, not a design choice) and printed to stdout from inside a shared
c  library whenever the subspace step needed backtracking. The backtracking
c  itself is unchanged -- only the printing is suppressed.
c        write(6,*) ' Positive dir derivative in projection '
c        write(6,*) ' Using the backtracking step '
      else
         go to 911
      endif
c
