! =======================================================================
! MTExpFit -- Native exponential-deceleration model fit, replacing R's
! `optim(method="L-BFGS-B")` call in `source_scripts/fitSpeed.r`.
!
! The model is  s(x) = a0 + aa*x - bb*exp(cc*x)  with parameter vector
! p = (a0, aa, bb, cc). The fit minimises the sum of squared residuals
!   f(p) = sum( (y - (p1 + p2*x - p3*exp(p4*x)))^2 )
! subject to box bounds, using the vendored reference L-BFGS-B 3.0
! optimizer (`setulb`, reverse communication) with an analytic gradient.
!
! The cost and gradient transcribe `fitSpeed.r` verbatim; the
! reverse-communication driver and the convergence-code mapping mirror
! R's `src/appl/lbfgsb.c` so a native call reproduces R's result.
! =======================================================================
module MTExpFit
...
! + aa*x - bb*exp(cc*x)
! with p = (a0, aa, bb, cc). The single site for the model formula; the cost,
! residual and Jacobian below all derive from it.
! -----------------------------------------------------------------------
...
! Mirrors `fitError` in fitSpeed.r.
! -----------------------------------------------------------------------
...
! These are R's `attr(rhs, "gradient")` columns for the nls Gauss-Newton
! step (NOT the cost gradient in ExpModelGrad, which is d(RSS)/dp).
! -----------------------------------------------------------------------
...
! minimise
! `ExpModelCost` under box bounds. Stateless: all L-BFGS-B working
! storage is local to this call.
!
! On return `coef` holds the final iterate, `fval` its cost, and
! `convergence` the R-optim code:
...
! -----------------------------------------------------------------------
subroutine ExpFitLBFGSB(ndata, x, y, seed, lower, upper, maxit, lmm, &
    ...
    do
        call setulb(NPARAM, lmm, coef, lower, upper, nbd, f, g, factr, &
        ...
        if (task(1:2) == 'FG') then
            ...
            ! Callers should pass maxit=300 here, NOT R's value of
            ! 100: this vendored L-BFGS-B 3.0 uses the corrected machine
            ! epsilon (epsilon(one) = 2.22e-16 in lbfgsb.f), whereas R's optim
            ! (L-BFGS-B 2.1) uses a legacy dpmeps that OVERestimates epsmch.
            ! The factr*epsmch stop is therefore strictly stricter here, so the
            ! same factr needs more iterations to converge (~101 vs R's ~89 on
            ! the canonical dataset) -- and lands on the identical minimum to
            ! <= 1e-8. maxit=100 would stop one step short (convergence=1).
            iter = iter + 1
            if (iter > maxit) then
                convergence = 1
                exit
...
! fit of the exp-deceleration
! model. This is the DOCUMENTED SUBSTITUTE for R's nls(algorithm="port")
! bounded fit: the vendor path (netlib PORT dn2gb) was declined because the
! PORT library is licensed for non-commercial use only (AT&T Bell Labs
! 1984) and this repository ships binaries -- see README.vendored. It runs
! the same Gauss-Newton iteration as the unbounded
! ExpFitGaussNewton below, but every trial iterate is PROJECTED onto the
! box [lower, upper] (each candidate p + factor*delta is clamped
! component-wise into the bounds). On a problem whose optimum is interior to
! the box -- the meteor speed-fit case -- projected GN and R's port
! trust-region-reflective method converge to the same minimum; they can
! differ only on samples where a bound is active at the optimum (expected
! negligible in practice). Stateless: all working storage is local to this
! call.
!
...
! gives R's relative-offset convergence criterion, evaluated BEFORE stepping,
! exactly as R's nlsModel$convCrit:
!     conv = sqrt( sum(rr(1:p)^2) / sum(rr(p+1:n)^2) ) < tol
! (with scaleOffset = 0, the nls.control default). rr(1:p) is the residual
...
! while a positive numerator over a zero denominator is R's +Inf and does not.
! The step solves
! R*delta = rr(1:p) (i.e. delta = (J^T J)^-1 J^T r) and is halved (starting
! from the running factor, R's nls carry-over) until the residual sum of
! squares does not increase; if the factor falls below minFactor the fit is
! declared non-converged (R errors "step factor ... reduced below
! 'minFactor'").
!
! Convergence codes: 0 converged; 1 iteration limit reached (R errors
! "number of iterations exceeded maximum"); 2 step factor below minFactor;
! 3 singular Jacobian / LAPACK failure (R errors "singular gradient"). On any
...
! sigma = sqrt(RSS/(n-4)), reproducing summary(nls)$coefficients and $sigma.
! -----------------------------------------------------------------------
subroutine ExpFitGaussNewtonBounded(ndata, x, y, seed, lower, upper, maxit, &
        tol, minFactor, coef, se, sigma, convergence, status)
    ...
    dof = ndata - NPARAM
    ! Project the seed onto the box so the iteration starts feasible (R's port
    ! errors if the start is outside the bounds; we clamp instead).
    ...
    do iter = 1, maxit
        ...
        rr = resid
        call dormqr('L', 'T', ndata, 1, NPARAM, jac, ndata, tau, rr, ndata, &
            work, lwork, info)
        if (info /= 0) then
            convergence = 3
            coef = p
        ...
        ! R's relative-offset criterion sqrt(sum(rr(1:p)^2)/sum(rr(p+1:n)^2)) < tol
        ! on rr = Q^T*resid, checked BEFORE stepping (R's nlsModel$convCrit).
        ...
        if (orthss <= 0.0_real64) then
            if (projss <= 0.0_real64) then
                ...
                ! (R evaluates NaN here and keeps iterating to its iteration limit;
                ! reporting convergence at an exact fit is the better answer.)
                conv = 0.0_real64
            else
                ...
                ! reduce it. R's criterion is +Inf here and does NOT converge --
                ! treating it as converged would report success at a non-solution.
        ...
        p = newp
        dev = newdev
        ! R's nls relaxes the step cap back toward a full step after a success.
...
! model, reproducing
! R's nls() DEFAULT (Gauss-Newton) algorithm -- the 'default' entry of
! fitSpeed.r's NLS_ALGS. Stateless: all working storage is local to this
! call.
!
! Each iteration builds the analytic Jacobian J = d(model)/dp (columns
! 1, x, -exp(cc*x), -bb*x*exp(cc*x); see ExpModelJacobian) and the residual
...
! exactly as R's nlsModel$convCrit:
!     conv = sqrt( sum(rr(1:p)^2) / sum(rr(p+1:n)^2) ) < tol
! (with scaleOffset = 0, the nls.control default). The step solves
! R*delta = rr(1:p) (i.e. delta = (J^T J)^-1 J^T r) and is halved (starting
! from the running factor, R's nls carry-over) until the residual sum of
! squares does not increase; if the factor falls below minFactor the fit is
! declared non-converged (R errors "step factor ... reduced below
...
! Gauss-Newton.
! -----------------------------------------------------------------------
subroutine ExpFitGaussNewton(ndata, x, y, seed, maxit, tol, minFactor, &
    ...
    call ExpFitGaussNewtonBounded(ndata, x, y, seed, lower, upper, maxit, &
        tol, minFactor, coef, se, sigma, convergence, status)
